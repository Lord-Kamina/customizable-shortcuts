...
let Windows = exports.Windows = {
	...
	handleKeyPress: function (event) {
		...
		if (shortcut.isComplete()) { // check if this is a custom shortcut
		    ...
		    else { overlay = Overlays.findByOverriddenShortcut(shortcut); } // check if this is a shortcut that is overriden by a custom one
    		if (overlay) {
    		event.preventDefault();
    		event.stopPropagation();
    		}
    		else {
    		overlay = Overlays.findByDisabledShortcut(shortcut);
...
sdkWindows.on("open",function (window) {
	...
	if (utils.isBrowser(window)) {
		...
		window.setTimeout(function () { 
			Windows.addEventListener("keypress", Windows.handleKeyPress, false)
...
sdkWindows.on("close",function (window) {
	window = viewFor(window);
	if (utils.isBrowser(window)) {
		window.removeEventListener("keypress", Windows.handleKeyPress, false);
    }
});