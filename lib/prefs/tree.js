...
const { prompterService, windowMediator } = require("../util/functions");
...
//     this.window.addEventListener("keypress", function(event) { self._onKeyPress(event) }, true);
    this.element = this.window.document.getElementById("ttCustomizableShortcuts_Tree");
    
    this.element.addEventListener("keydown", function(event) { self._onKeyPress(event) }, true);
...
PreferenceTree.prototype = {
	...
	_onKeyPress: function(event) {
        ...
        shortcut.isComplete() && (this.element.inputField.value = shortcut.toString());
    	...
    	const { editingRow, editingColumn } = this.element;
...
customXULTree.prototype = {
    ...
    _onKeyDown: function(event) {
        ...
        if (("Enter" == event.key) || ("VK_ENTER" == event.keycode) || ("VK_RETURN" == event.keycode)) {            
				...
				switch (this.element.editingColumn.id) {
					...
					break;
    	 			...
    	 			case "KeybinderCustomXUL_ActionCol":
    	 				...
    	 				let mainWindow = windowMediator.getMostRecentWindow("navigator:browser");
    	 				if (!mainWindow.document.querySelector(`command[id="${mapItemCommand}"]`)) { 
    	 					prompterService.alert(this.window, _("conflictWarning.title"),_("customXulBadCommand.text"));
						    this.element.startEditing(row,column);
						    return
    	 				}
			...
			CustomXUL.storeKeys(); // Array updating code here.
    		...
    		case "KeybinderCustomXUL_NameCol":
    			let filter = this.window.document.getElementById("KeybinderCustomXUL_Filter");
    	 		...
    	 		if (CustomXUL.find(mapItemKey)) {
					if (mapItemKey != rowKey) {
						prompterService.alert(this.window, _("conflictWarning.title"), _("customXulConflict.text")); // Break in case name entered already exists, but only display a warning if it would have resulted in a duplicate.
						this.element.startEditing(row,target);
						return
					}
					break;
    	 		}
