...
function keybinderDialog (id, uri, name, options, extras = null) {
	if (!Cc["@mozilla.org/appshell/window-mediator;1"].getService(Ci.nsIWindowMediator).getMostRecentWindow(id)) {
		Windows.getMostRecentWindow().openDialog(uri, name, options, extras);
	}
	else {
		Cc["@mozilla.org/appshell/window-mediator;1"].getService(Ci.nsIWindowMediator).getMostRecentWindow(id).focus();
	} 
}
