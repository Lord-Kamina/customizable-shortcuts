...
events.on("toplevel-window-ready",function(event) {
	...
	else if ("KeybinderCustomXULDialog" == id) {
...
keybinderDialog('Keybinder:config', 'chrome://keybinder/content/ttShortcuts_preferences.xul', 'Keybinder', 'chrome,titlebar=yes,close=no,toolbar=no,centerscreen,dialog=yes');
});

extensionPrefs.on("customXULDialog", function() {
keybinderDialog('Keybinder:CustomXUL', 'chrome://keybinder/content/customXulKeys.xul', 'CustomXUL', 'chrome,titlebar=yes,close=no,toolbar=no,centerscreen,dialog=yes');
});
...
let {TreeView} = require("../prefs/treeview");
    ...
    this.acceptButton.addEventListener("command", function() {
        var { windowMediator } = require("../util/functions");
...
function customXULDialogInit (window) {
 let {customXULTreeView} = require("../prefs/treeview");
    let self = this;
    this.window = window;
