    ...
    onTrack: function(window) {
        if ("main-window" == window.document.documentElement.getAttribute("id")) {
            if (!require("sdk/private-browsing").isPrivate(window)) // Do not allow access to preferences in Private Windows.           
        	    if (!window.document.getElementById("ttShortcutsMenuItem")) {
	        	    new MenuItem(window);
    	        	if ("Darwin" == require("../util/os").checkOSID()) new MenuItem(window, "tabSwitching"); // Bug 406199: Add "Next/Previous Tab"            
            }
 items to the "Window" Menu if on Mac.
...
if (!Cc["@mozilla.org/appshell/window-mediator;1"].getService(Ci.nsIWindowMediator).getMostRecentWindow('Keybinder:URLPatterns')) {
