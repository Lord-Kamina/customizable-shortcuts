...
let {Groups} = require("../prefs/groups.js");
let {Actions} = require("../prefs/actions.js");
...
let TreeView = function(filter) {
    ...
    if (filter) {
        ...
        keys = keys.filter(function(key) {
            return (-1 < Actions.findByKey(key).toLowerCase().indexOf(filter) || -1 < key.shortcut.toString().toLowerCase().indexOf(filter))
        });
    }
...
TreeView.prototype = {
    ...
    getCellText: function(idx, column) {
        ...
        let key = row.key;
        if (!column.index) return Actions.findByKey(key);
