...
//                     	console.warn("[ADDON SCRIPT] And this time, we shouldn't keep looking...");
                ...
                worker.port.on("disposeOfMinion", function(payload) {
                		let workerID = payload['ID'];
//                 		console.warn("Debugging a worker... "+JSON.stringify(worker,null,2));
//                 		console.warn("Debugging a payload... "+JSON.stringify(payload,null,2));
						worker.destroy(workerID);
                });
                
                worker.on('detach', function(target = workerID) {
                	console.warn("Do we have a worker ID?: "+target);
                    delete pageWorkers[target]; // Kill useless workers.
