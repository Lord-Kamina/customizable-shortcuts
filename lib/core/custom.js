...
let storeKeys = exports.storeKeys = function() {
	let array = [];
	for (let [mapKey, mapValue] of allCustomKeys().entries()) {
		array.push({"key":mapValue.key, "label":mapValue.label, "command":mapValue.command});
	}
	extensionPrefs.prefs['customXULKeys'] = JSON.stringify(array,null,2);
}
