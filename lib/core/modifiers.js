...
const modifierNames = {
    control: _("ctrlKey"),
    meta: function() { let tmpString = ""; if ("darwin" == system.platform) {  tmpString = _("cmdKey"); } else if ("winnt" == system.platform) { tmpString = _("winKey"); } else if ("linux" == system.platform) { tmpString = _("superKey"); } return tmpString; },
    shift: _("shiftKey"),
    alt: _("altKey"),
    os: _("osKey"),
    altgr: _("altgrKey")
...
Modifiers.fromEvent = function(event) {
	    ...
	    if (getModifierState(event,"Control") || getModifierState(event, "AltGraph")) { modifiers.push("control"); }
    	if (getModifierState(event, "Shift")) { modifiers.push("shift"); }
		if (getModifierState(event, "Meta") || getModifierState(event, "OS")) {
		modifiers.push("meta");
		}
		if (getModifierState(event, "Alt") || event.getModifierState("AltGraph")) { modifiers.push("alt"); }

    	if (event.keyCode in modifierKeys) {
        	let key = modifierKeys[event.keyCode];
	        if (-1 == modifiers.indexOf(key)) { modifiers.push(key); }
	    }
